...
namespace TextRpg
{
    class Dungeon
    {
        ...
        private void SelectDungeon()
        {
            ...
            int highestAllowedStage = Program.player.highestClearedStage + 1; // 최고 클리어 단계 + 1까지만 도전 가능

            for (int i = 1; i <= Math.Min(maxStage, highestAllowedStage); i++)
            {
                if (i <= Program.player.highestClearedStage)
                {
                    // 이미 클리어한 단계는 별표로 표시
                    dungeonOptions.Add($"{i}단계 던전 [yellow]★[/]");
                }
                else
                {
                    dungeonOptions.Add($"{i}단계 던전");
                }
            }

            dungeonOptions.Add("\n0. 돌아가기");

            var dungeonChoice = AnsiConsole.Prompt(
                new SelectionPrompt<string>()
                .Title("도전할 던전 단계를 선택하세요")
                .PageSize(Math.Min(12, maxStage + 1))
                .AddChoices(dungeonOptions)
                .WrapAround());
            ...
            // 선택한 단계 추출 (예: "1단계 던전" 또는 "1단계 던전 ★" -> 1)
            string stageText = dungeonChoice.Split('단')[0];
            int selectedStage = int.Parse(stageText);

            // 선택한 단계 던전 입장
            SoundManager.Instance.StopMusic();
            EnterDungeon(selectedStage);
        }
        private void EnterDungeon(int selectedStage)
        {
            ...
            // 선택한 단계의 던전만 생성
            _stages.Add(new Stage($"{selectedStage}단계", MonsterFactory.Create(selectedStage)));
            ...
            foreach (Stage stage in _stages)
            {
                ...
                // 스테이지 클리어 후 최고 클리어 단계 업데이트
                if (selectedStage > Program.player.highestClearedStage)
                {
                    Program.player.highestClearedStage = selectedStage;
                    AnsiConsole.MarkupLine($"[#ffff00]축하합니다! 새로운 최고 기록: {selectedStage}단계 클리어![/]");
                }

                // 스테이지 클리어 메시지 표시
                Console.Clear();
                AnsiConsole.MarkupLine($"[#6cf540]{stage.Name}[/] 스테이지를 클리어했습니다!");
                Thread.Sleep(1500);
                SoundManager.Instance.StopMusic();
                SoundManager.Instance.StartMainMusic();
            }
